...
class Dict : public DictBase<K, V> {
 ...
 public:
  ...
  V operator[](K key) {
    ...
    unsigned int position = 0;    DictSlot<K, V>* slot = GetSlotByKey(_DictSlots_ref, key, position);
 ...
 public:
  ...
  Matrix<X>* ToMatrix() {
    ...
    for (DictIteratorBase<K, V> i(Begin()); i.IsValid(); ++i) result[i.Index()] = (X)i.Value();
