...
class Indicator : public IndicatorBase {
  ...
  */
   ...
   */
  void SetParams(const IndicatorParams& _iparams) {
    iparams.name = _iparams.name;
    iparams.shift = _iparams.shift;
    iparams.max_buffers = _iparams.max_buffers;
    iparams.max_modes = _iparams.max_modes;
    iparams.max_params = _iparams.max_params;
    iparams.tf = _iparams.tf;
    iparams.itype = _iparams.itype;
    iparams.idstype = _iparams.idstype;
    iparams.idvrange = _iparams.idvrange;
    iparams.dtype = _iparams.dtype;
    iparams.indi_color = _iparams.indi_color;
    iparams.indi_data_source_id = _iparams.indi_data_source_id;
    iparams.indi_data_source_mode = _iparams.indi_data_source_mode;
    iparams.is_draw = _iparams.is_draw;
    iparams.draw_window = _iparams.draw_window;
    iparams.custom_indi_name = _iparams.custom_indi_name;
    // Copy input params array.
    ArrayResize(iparams.input_params, ArraySize(_iparams.input_params));
    for (int i = 0; i < ArraySize(_iparams.input_params); ++i) {
      iparams.input_params[i] = _iparams.input_params[i];
    }
  }
