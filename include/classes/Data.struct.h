...
struct DataParamEntry : public MqlParam {
  // Added for MetaEditor build 5660+: keep default ctor
  DataParamEntry() {}
  // Prefer ctors over deprecated init-via-operator=
  DataParamEntry(const long& v) { *this = v; }
  // Added for MetaEditor build 5660+: prefer constructors over deprecated init-via-operator=
  DataParamEntry(const double& v) { *this = v; }
  DataParamEntry(const string& v) { *this = v; }
